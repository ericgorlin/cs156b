...
SGD2::SGD2(int lf, double lambda_val, double lr)
{
    ...
    user_vec.assign(n_users, 0);
    movie_vec.assign(n_movies, 0);
    clock_t end = clock();
...
void SGD2::run_sgd()
{
    ...
    double lr = learn_rate; // learn rate
    int user;
    int movie;
    int rating;
    double estimate;
    ...
    for (unsigned int epoch = 1; epoch < 50; epoch++) {
        ...
        for (unsigned int i = 0; i < 98291669; i++) { //< y.n_cols
            ...
            user_vec[user] += lr * (error - lambda * user_vec[user]);
            movie_vec[movie] += lr * (error - lambda * movie_vec[movie]);
        ...
        if (old_error - new_error <= .001 && epoch > 5) {
            break;
        }
        old_error = new_error;
    }
...
/*
int main() {
    SGD2 sgd(30, 0.01, 0.007); // remember to have learning rate divided by number of epochs
